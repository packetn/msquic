...
typedef struct QUIC_LIBRARY {
  //
  // Tracks whether the library loaded (DllMain or DriverEntry invoked on
  // Windows).
  //
  BOOLEAN Loaded : 1;
  ...
  //
  _Field_range_(>, 0) uint16_t ProcessorCount;

  //
  // Number of partitions currently being used.
  //
  _Field_range_(>, 0) uint16_t PartitionCount;
  ...
  //
  _Field_range_(0, MSQUIC_MAX_CID_SID_LENGTH) uint8_t CidServerIdLength;
  // uint8_t CidPartitionIdLength; // Currently hard coded
  // (MSQUIC_CID_PID_LENGTH)
  _Field_range_(QUIC_MIN_INITIAL_CONNECTION_ID_LENGTH,
                MSQUIC_CID_MAX_LENGTH) uint8_t CidTotalLength;
  ...
  //
  _Field_size_(PartitionCount) QUIC_LIBRARY_PP* PerProc;
...
_IRQL_requires_max_(DISPATCH_LEVEL)
    _Ret_range_(0, MsQuicLib.PartitionCount - 1) inline uint16_t
    QuicLibraryGetCurrentPartition(void) {
  return ((uint16_t)CxPlatProcCurrentNumber()) % MsQuicLib.PartitionCount;
}

_IRQL_requires_max_(DISPATCH_LEVEL) inline uint16_t
    QuicPartitionIdCreate(uint16_t BaseIndex) {
...
_IRQL_requires_max_(DISPATCH_LEVEL) inline uint16_t
    QuicPartitionIdGetIndex(uint16_t PartitionId) {
  return (PartitionId & MsQuicLib.PartitionMask) % MsQuicLib.PartitionCount;
}

_IRQL_requires_max_(DISPATCH_LEVEL) inline uint16_t
    QuicPartitionIndexIncrement(uint16_t PartitionIndex, uint16_t Increment) {
  CXPLAT_DBG_ASSERT(Increment < MsQuicLib.PartitionCount);
  return (PartitionIndex + Increment) % MsQuicLib.PartitionCount;
}

_IRQL_requires_max_(DISPATCH_LEVEL) inline uint16_t
    QuicPartitionIndexDecrement(uint16_t PartitionIndex, uint16_t Decrement) {
...
_IRQL_requires_max_(DISPATCH_LEVEL) inline void QuicPerfCounterAdd(
    _In_ QUIC_PERFORMANCE_COUNTERS Type, _In_ int64_t Value) {
  CXPLAT_DBG_ASSERT(Type >= 0 && Type < QUIC_PERF_COUNTER_MAX);
  uint32_t ProcIndex = CxPlatProcCurrentNumber() % MsQuicLib.ProcessorCount;
  CXPLAT_DBG_ASSERT(ProcIndex < (uint32_t)MsQuicLib.PartitionCount);
  InterlockedExchangeAdd64(&(MsQuicLib.PerProc[ProcIndex].PerfCounters[Type]),
                           Value);
}
...
_IRQL_requires_max_(DISPATCH_LEVEL) void QuicPerfCounterSnapShot(
    _In_ uint64_t TimeDiffUs);

_IRQL_requires_max_(DISPATCH_LEVEL) inline void QuicPerfCounterTrySnapShot(
  ...
  if ((int64_t)TimeLast !=
      InterlockedCompareExchange64((int64_t*)&MsQuicLib.PerfCounterSamplesTime,
                                   (int64_t)TimeNow, (int64_t)TimeLast)) {
...
// path.
//
inline _Success_(return != NULL) QUIC_CID_HASH_ENTRY* QuicCidNewRandomSource(
    _In_opt_ QUIC_CONNECTION* Connection,
    _In_reads_opt_(MsQuicLib.CidServerIdLength) const void* ServerID,
    _In_ uint16_t PartitionID, _In_ uint8_t PrefixLength,
    _In_reads_(PrefixLength) const void* Prefix) {
  CXPLAT_DBG_ASSERT(MsQuicLib.CidTotalLength <=
                    QUIC_MAX_CONNECTION_ID_LENGTH_V1);
  CXPLAT_DBG_ASSERT(MsQuicLib.CidTotalLength == MsQuicLib.CidServerIdLength +
                                                    MSQUIC_CID_PID_LENGTH +
                                                    MSQUIC_CID_PAYLOAD_LENGTH);
  CXPLAT_DBG_ASSERT(MSQUIC_CID_PAYLOAD_LENGTH > PrefixLength);

  QUIC_CID_HASH_ENTRY* Entry = (QUIC_CID_HASH_ENTRY*)CXPLAT_ALLOC_NONPAGED(
  ...
  if (Entry != NULL) {
    ...
    CXPLAT_STATIC_ASSERT(MSQUIC_CID_PID_LENGTH == sizeof(PartitionID),
                         "Assumes a 2 byte PID");
...
_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS
    QuicLibrarySetGlobalParam(_In_ uint32_t Param, _In_ uint32_t BufferLength,
                              _In_reads_bytes_(BufferLength)
                                  const void* Buffer);

_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS QuicLibraryGetGlobalParam(
    _In_ uint32_t Param, _Inout_ uint32_t* BufferLength,
    _Out_writes_bytes_opt_(*BufferLength) void* Buffer);

_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS
    QuicLibrarySetParam(_In_ HQUIC Handle, _In_ QUIC_PARAM_LEVEL Level,
                        _In_ uint32_t Param, _In_ uint32_t BufferLength,
                        _In_reads_bytes_(BufferLength) const void* Buffer);

_IRQL_requires_max_(PASSIVE_LEVEL) QUIC_STATUS
    QuicLibraryGetParam(_In_ HQUIC Handle, _In_ QUIC_PARAM_LEVEL Level,
                        _In_ uint32_t Param, _Inout_ uint32_t* BufferLength,
                        _Out_writes_bytes_opt_(*BufferLength) void* Buffer);
...
_IRQL_requires_max_(DISPATCH_LEVEL) BOOLEAN
    QuicLibraryTryAddRefBinding(_In_ QUIC_BINDING* Binding);
...
// possibly even crash!
//
_IRQL_requires_max_(PASSIVE_LEVEL) void QuicLibraryReleaseBinding(
...
_IRQL_requires_max_(PASSIVE_LEVEL) BOOLEAN
    QuicLibraryOnListenerRegistered(_In_ QUIC_LISTENER* Listener);

//
// Returns the next available worker. Note, the worker may be overloaded.
//
_IRQL_requires_max_(DISPATCH_LEVEL) QUIC_WORKER* QuicLibraryGetWorker(
...
//
_IRQL_requires_max_(DISPATCH_LEVEL)
    _Ret_maybenull_ CXPLAT_KEY* QuicLibraryGetCurrentStatelessRetryKey(void);

//
// Returns the stateless retry key for that timestamp.
//
_IRQL_requires_max_(DISPATCH_LEVEL)
    _Ret_maybenull_ CXPLAT_KEY* QuicLibraryGetStatelessRetryKeyForTimestamp(
        _In_ int64_t Timestamp);

//
// Called when a new (server) connection is added in the handshake state.
//
_IRQL_requires_max_(DISPATCH_LEVEL) void QuicLibraryOnHandshakeConnectionAdded(
    void);
...
//
_IRQL_requires_max_(
    DISPATCH_LEVEL) void QuicLibraryOnHandshakeConnectionRemoved(void);
