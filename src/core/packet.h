...
#define PtkConnPre(Connection) \
  (Connection == NULL ? '-'    \
                      : (PacketLogPrefix[0][QuicConnIsServer(Connection)]))
...
#define MIN_INV_LONG_HDR_LENGTH \
  (sizeof(QUIC_HEADER_INVARIANT) + sizeof(uint8_t))
...
_IRQL_requires_max_(DISPATCH_LEVEL) _Success_(return != FALSE) BOOLEAN
    QuicPacketValidateInvariant(
        ...
        _Inout_ CXPLAT_RECV_PACKET* Packet, _In_ BOOLEAN IsBindingShared);
...
#define MIN_LONG_HEADER_LENGTH_V1                                    \
  (sizeof(QUIC_LONG_HEADER_V1) + sizeof(uint8_t) + sizeof(uint8_t) + \
   4 * sizeof(uint8_t))
...
//
#define MIN_RETRY_HEADER_LENGTH_V1 (sizeof(QUIC_RETRY_V1) + sizeof(uint8_t))
...
#define MIN_SHORT_HEADER_LENGTH_V1 \
  (sizeof(QUIC_SHORT_HEADER_V1) + 4 * sizeof(uint8_t))
...
//
inline BOOLEAN QuicPacketIsHandshake(_In_ const QUIC_HEADER_INVARIANT* Packet) {
...
_IRQL_requires_max_(DISPATCH_LEVEL) _Success_(return != FALSE) BOOLEAN
    QuicPacketValidateLongHeaderV1(
        _In_ const void* Owner,  // Binding or Connection depending on state
        _In_ BOOLEAN IsServer, _Inout_ CXPLAT_RECV_PACKET* Packet,
...
inline _IRQL_requires_max_(DISPATCH_LEVEL) void QuicPktNumDecode(
    _In_range_(1, 4) uint8_t PacketNumberLength,
    _In_reads_bytes_(PacketNumberLength) const uint8_t* Buffer,
...
// given low bytes.
//
inline _IRQL_requires_max_(DISPATCH_LEVEL) uint64_t
    QuicPktNumDecompress(_In_ uint64_t ExpectedPacketNumber,
  ...
  const uint64_t Mask = 0xFFFFFFFFFFFFFFFFULL
                        << ((uint64_t)(8 * CompressedPacketNumberBytes));
  const uint64_t PacketNumberInc = (~Mask) + 1;
  uint64_t PacketNumber =
      (Mask & ExpectedPacketNumber) | CompressedPacketNumber;
...
//
inline _IRQL_requires_max_(DISPATCH_LEVEL) _Success_(return != 0) uint16_t
    QuicPacketEncodeLongHeaderV1(
        ...
        _In_ const QUIC_CID* const SourceCid, _In_ uint16_t TokenLength,
        _In_reads_opt_(TokenLength) const uint8_t* const Token,
  ...
  uint16_t RequiredBufferLength =
      sizeof(QUIC_LONG_HEADER_V1) + DestCid->Length + sizeof(uint8_t) +
      SourceCid->Length +
  ...
  if (PacketType == QUIC_INITIAL) {
    RequiredBufferLength +=
        QuicVarIntSize(TokenLength) + TokenLength;  // TokenLength
  }
...
#define QuicPacketMaxBufferSizeForRetryV1()                           \
  MIN_RETRY_HEADER_LENGTH_V1 + 3 * QUIC_MAX_CONNECTION_ID_LENGTH_V1 + \
...
// Encodes the long header fields.
//
_IRQL_requires_max_(DISPATCH_LEVEL) _Success_(return != 0) uint16_t
    QuicPacketEncodeRetryV1(
        ...
        _In_reads_(OrigDestCidLength) const uint8_t* const OrigDestCid,
        _In_ uint8_t OrigDestCidLength, _In_ uint16_t TokenLength,
        _In_reads_(TokenLength) uint8_t* Token, _In_ uint16_t BufferLength,
        _Out_writes_bytes_(BufferLength) uint8_t* Buffer);
...
//
inline _IRQL_requires_max_(DISPATCH_LEVEL) _Success_(return != 0) uint16_t
    QuicPacketEncodeShortHeaderV1(_In_ const QUIC_CID* const DestCid,
                                  _In_ uint64_t PacketNumber,
                                  _In_ uint8_t PacketNumberLength,
                                  _In_ BOOLEAN SpinBit, _In_ BOOLEAN KeyPhase,
                                  _In_ uint16_t BufferLength,
  ...
  uint16_t RequiredBufferLength =
      sizeof(QUIC_SHORT_HEADER_V1) + DestCid->Length + PacketNumberLength;
  if (BufferLength < RequiredBufferLength) {
    return 0;
  }
...
inline uint32_t QuicPacketHash(_In_ const QUIC_ADDR* const RemoteAddress,
                               _In_ uint8_t RemoteCidLength,
                               _In_reads_(RemoteCidLength)
                                   const uint8_t* const RemoteCid) {
  uint32_t Key = 0, Offset;
  CxPlatToeplitzHashComputeAddr(&MsQuicLib.ToeplitzHash, RemoteAddress, &Key,
                                &Offset);
  if (RemoteCidLength != 0) {
    Key ^= CxPlatToeplitzHashCompute(
        &MsQuicLib.ToeplitzHash, RemoteCid,
        CXPLAT_MIN(RemoteCidLength, QUIC_MAX_CONNECTION_ID_LENGTH_V1), Offset);
  }
  return Key;
}
...
//
_IRQL_requires_max_(DISPATCH_LEVEL) void QuicPacketLogHeader(
    _In_opt_ QUIC_CONNECTION* Connection, _In_ BOOLEAN Rx,
    _In_ uint8_t CIDLength, _In_ uint64_t PacketNumber,
    _In_ uint16_t PacketLength,
    _In_reads_bytes_(PacketLength) const uint8_t* const Packet,
    _In_ uint32_t Version  // Network Byte Order. Used for Short Headers
);

_IRQL_requires_max_(DISPATCH_LEVEL) void QuicPacketLogDrop(
    _In_ const void* Owner,  // Binding or Connection depending on state
    _In_ const CXPLAT_RECV_PACKET* Packet, _In_z_ const char* Reason);

_IRQL_requires_max_(DISPATCH_LEVEL) void QuicPacketLogDropWithValue(
    _In_ const void* Owner,  // Binding or Connection depending on state
    _In_ const CXPLAT_RECV_PACKET* Packet, _In_z_ const char* Reason,
    _In_ uint64_t Value);
