...
#else
#define QUIC_LOCALHOST_FOR_AF(Af) \
  ((Af == QUIC_ADDRESS_FAMILY_INET) ? "localhost" : "ip6-localhost")
#endif
...
#define QUIC_CERTIFICATE_FLAG_IGNORE_CERTIFICATE_CN_INVALID \
  0x00001000  // bad common name in X509 Cert.
#define QUIC_CERTIFICATE_FLAG_IGNORE_CERTIFICATE_DATE_INVALID \
  0x00002000  // expired X509 Cert.
...
inline BOOLEAN QuicAddrFamilyIsValid(_In_ QUIC_ADDRESS_FAMILY Family) {
  return Family == QUIC_ADDRESS_FAMILY_UNSPEC ||
         Family == QUIC_ADDRESS_FAMILY_INET ||
         Family == QUIC_ADDRESS_FAMILY_INET6;
}

inline BOOLEAN QuicAddrIsValid(_In_ const QUIC_ADDR* const Addr) {
  return QuicAddrFamilyIsValid(Addr->Ip.sa_family);
}

inline BOOLEAN QuicAddrCompareIp(_In_ const QUIC_ADDR* const Addr1,
                                 _In_ const QUIC_ADDR* const Addr2) {
  if (QUIC_ADDRESS_FAMILY_INET == Addr1->Ip.sa_family) {
    return memcmp(&Addr1->Ipv4.sin_addr, &Addr2->Ipv4.sin_addr,
                  sizeof(IN_ADDR)) == 0;
  } else {
    return memcmp(&Addr1->Ipv6.sin6_addr, &Addr2->Ipv6.sin6_addr,
                  sizeof(IN6_ADDR)) == 0;
  }
}

inline BOOLEAN QuicAddrCompare(_In_ const QUIC_ADDR* const Addr1,
                               _In_ const QUIC_ADDR* const Addr2) {
...
inline QUIC_ADDRESS_FAMILY QuicAddrGetFamily(_In_ const QUIC_ADDR* const Addr) {
  return Addr->Ip.sa_family;
}

inline void QuicAddrSetFamily(_In_ QUIC_ADDR* Addr,
                              _In_ QUIC_ADDRESS_FAMILY Family) {
  Addr->Ip.sa_family = Family;
}

inline uint16_t QuicAddrGetPort(_In_ const QUIC_ADDR* const Addr) {
  if (!Addr) return 0;
...
inline void QuicAddrSetPort(_Out_ QUIC_ADDR* Addr, _In_ uint16_t Port) {
  if (!Addr) return;
  if (QUIC_ADDRESS_FAMILY_INET == Addr->Ip.sa_family) {
...
//
inline void QuicAddrIncrement(_Inout_ QUIC_ADDR* Addr) {
...
inline void QuicAddrSetToLoopback(_Inout_ QUIC_ADDR* Addr) {
  if (Addr->Ip.sa_family == QUIC_ADDRESS_FAMILY_INET) {
...
inline uint32_t QUIC_NO_SANITIZE("unsigned-integer-overflow")
    QuicAddrHash(_In_ const QUIC_ADDR* Addr) {
...
inline BOOLEAN QuicAddrIsWildCard(_In_ const QUIC_ADDR* const Addr) {
...
inline BOOLEAN QuicAddr4FromString(_In_z_ const char* AddrStr,
...
inline BOOLEAN QuicAddr6FromString(_In_z_ const char* AddrStr,
                                   _Out_ QUIC_ADDR* Addr) {
  if (AddrStr[0] == '[') {
...
inline BOOLEAN QuicAddrFromString(_In_z_ const char* AddrStr,
                                  _In_ uint16_t Port,  // Host byte order
                                  _Out_ QUIC_ADDR* Addr) {
  Addr->Ipv4.sin_port = htons(Port);
  return QuicAddr4FromString(AddrStr, Addr) ||
...
inline BOOLEAN QuicAddrToString(_In_ const QUIC_ADDR* Addr,
  ...
  if (inet_ntop(
          ...
          Address, sizeof(QUIC_ADDR_STR)) == NULL) {
    return FALSE;
  }
